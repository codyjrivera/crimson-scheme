...
static void skipMultilineComment(std::istream& stream, long& line,
                          long& col);
...
Lexer::Lexer(const Lexer& lex)
    : inStream(lex.inStream), line(lex.line), col(lex.col),
      nextToken(lex.nextToken), saveToken(lex.saveToken)
{
}
...
void Lexer::setStream(std::istream& stream) { inStream = &stream; flush(); }
