...
Exp* Parser::parseFile(Lexer& lex) {
    try {
        ...
        Exp* newExp =
            new Exp((new Exp(Data::Symbol(std::string("begin")))), (Exp*)NULL);
        ...
        while (parsedExp != NULL) {
            ...
            currentExp->setRight(new Exp(parsedExp, (Exp*)NULL,
                                         parsedExp->getLine(),
                                         parsedExp->getCol()));
        ...
        if (tok.getType() != TokenType::END) {
            throw InterpreterError(
                "Unexpected Token: ' " + tok.getValue() + " '", tok.getLine(),
                tok.getCol());
        }
        return newExp;
...
Exp* Parser::parsePrimitive(Lexer& lex) {
    ...
    switch (tok.getType()) {
        case TokenType::BOOLEAN:
            // To mirror Scheme's rule of truthiness, a false token is the only
            // token that evaluates to false
            if (tok.getValue() == "#f") {
                newExp =
                    new Exp(Data::Boolean(false), tok.getLine(), tok.getCol());
                allocNodes.push_back(newExp);
                return newExp;
            } else {
                newExp =
                    new Exp(Data::Boolean(true), tok.getLine(), tok.getCol());
                allocNodes.push_back(newExp);
                return newExp;
            }
            break;
        ...
        case TokenType::IDENTIFIER:
            ...
            break;
        default:
            throw InterpreterError("Corrupt Token: ' " + tok.getValue() + " '",
...
Exp* Parser::parseNumber(Lexer& lex) {
    ...
    if (tok.getValue().find('.') == std::string::npos &&
        strNumStream >> intValue) {
        ...
        if (strNumStream >> floatValue) {
            newExp =
                new Exp(Data::Float(floatValue), tok.getLine(), tok.getCol());
            allocNodes.push_back(newExp);
            return newExp;
        } else {
            throw InterpreterError(
                "Cannot Parse Number ' " + tok.getValue() + " '", tok.getLine(),
                tok.getCol());
        }
    }
}
...
Exp* Parser::parseQuote(Lexer& lex) {
    ...
    Token tok = lex.next();
    Exp* newExp =
        new Exp((new Exp(Data::Symbol("quote"), tok.getLine(), tok.getCol())),
...
Exp* Parser::parseHash(Lexer& lex) {
    ...
    Token tok = lex.next();
    Exp* newExp =
        new Exp((new Exp(Data::Symbol("hash"), tok.getLine(), tok.getCol())),
                (Exp*)NULL, tok.getLine(), tok.getCol());
    allocNodes.push_back(newExp);
    newExp->setRight(parseExp(lex));
    return newExp;
...
Exp* Parser::parseSExp(Lexer& lex) {
    ...
    if (tok.getType() == TokenType::PAREN && tok.getValue() == ")") {
        ...
        throw InterpreterError(
            "Expected ' ) ' but got: ' " + tok.getValue() + " '", tok.getLine(),
            tok.getCol());
    }
}
...
Exp* Parser::parseCompoundRest(Lexer& lex) {
    ...
    Exp* parsedExp = parseExp(lex);
    while (parsedExp != NULL) {
        currentExp->setRight(new Exp(
            parsedExp, (Exp*)NULL, parsedExp->getLine(), parsedExp->getCol()));
        allocNodes.push_back(currentExp->getRight());
        currentExp = currentExp->getRight();
        parsedExp = parseExp(lex);
    }
